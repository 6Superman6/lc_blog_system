...
public class BeforeBlogController {
    ...
    private static final String INPUT = "admin/blogs-add";  // 跳转到新增博客页面
    private static final String LIST = "blogs";          // 跳转到博客页面
    private static final String REDIRECT_LIST = "redirect:/before/blogs";  // 跳转到/admin/blogs接口
                        ...
                        BlogQuery blog,Model model,HttpServletRequest request)
    {
        ...
        model.addAttribute("page",blogService.myBlog(pageable,blog,user.getId()));  // 查看我的博客
        return LIST;
    }
                         ...
                         BlogQuery blog,Model model,HttpServletRequest request)
    {
        ...
        // 所以blogService中该方法中加入了组合条件，所以相当于调用的blogRepository.findAll(条件+我的博客);
        model.addAttribute("page",blogService.myBlog(pageable,blog,user.getId()));
        return "blogs :: blogList";  // blogs.html页面中有一个blogList属性表
    }
...
//        System.out.println("file: "+file);
        ...
        if(filename!=null&&!"".equals(filename)&&file!=null)
        {
            ...
            else  // 只要filename不为null，文件就上传成功了，所以上传的非图片文件要删除
            {
